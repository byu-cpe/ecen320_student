...
def main():
    ...
    tester.add_required_tracked_files(
        [
            "seven_segment.sv",
            "seven_segment_top.sv",
            "sim_seven_segment_top.tcl",
            "sim_seven_segment_top.png",
            "segment_lut.png",
        ]
    )
    tester.add_Makefile_rule(
        "sim_sevensegment", ["seven_segment.sv"], ["sim_sevensegment.log"]
    )
    tester.add_build_test(
        repo_test.FileRegexCheck(
            tester, "sim_sevensegment.log", "\*\*\* Error", "Testbench Log Test"
        )
    )
    tester.add_Makefile_rule("synth", [], ["synthesis.log", "seven_segment_synth.dcp"])
    tester.add_Makefile_rule(
        "implement",
        ["seven_segment_synth.dcp"],
        ["implement.log", "seven_segment.bit", "seven_segment.dcp", "utilization.rpt"],
    )
    tester.run_tests()
